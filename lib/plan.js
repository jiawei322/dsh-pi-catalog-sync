...
export function buildPlan({ route, entries, installed = {}, options = {} }) {
  ...
  if (entries.length === 0) {
    return { route, mode: 'none', reason: 'pi.dev lists no models for this route', routeModels: [], companionModels: [], dropped, warnings };
  }
  ...
  if (!mixed) {
    ...
    return { route, mode: 'in-place', routeModels: models, routeApi: undefined, companionModels: [], dropped, warnings };
  }

  if (strategy === 'skip') {
    warnings.push({ id: '*', route, reason: `mixed-protocol route (${[...apiSet].join(', ')}); strategy is "skip"` });
    return { route, mode: 'skip', routeModels: [], companionModels: [], dropped, warnings };
  }
  ...
  if (strategy === 'route-api') {
    ...
    return { route, mode: 'route-api', routeModels: models, routeApi: forcedApi, companionModels: [], dropped, warnings };
  }
  ...
  if (novelties.length === 0) {
    return { route, mode: 'companion', routeModels: [], companionModels: [], dropped, warnings };
  }
  ...
  if (companion === undefined) {
    ...
    return { route, mode: 'companion', routeModels: [], companionModels: [], dropped, warnings, companionSpec };
  }
  ...
  return { route, mode: 'companion', routeModels: knownModels, companionModels, dropped, warnings, companionSpec };
}
