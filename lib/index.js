...
export function apply(ctx) {
  ...
  let inFlight;
  const syncNow = (dryRun) => {
    inFlight ??= engine.syncNow({ dryRun }).finally(() => {
      inFlight = undefined;
    });
    return inFlight;
  };
  ...
  ctx.effect(() => {
    ...
    const schedule = () => {
      ...
      const handles = [setTimeout(() => void runRound('first'), config.startupDelaySeconds * 1000)];
    ...
    cmdCtx.effect(() => {
      ...
      return commands.register({
        ...
        handler: async (invocation) => {
          ...
          try {
            const { report } = await syncNow(dryRun);
